    ...
    <script>
        ...
        loginForm.addEventListener('submit', (e) => {
            ...
            if (username === 'iskconpali' && password === 'haribol') {
                auth.signInWithEmailAndPassword(username + "@iskcon.com", password)
                    .then(() => {
                        loginAdmin();
                    })
                    .catch(error => {
        ...
        uploadForm.addEventListener('submit', (e) => {
            ...
            if (fileInput.files.length > 0) {
                ...
                const storageRef = storage.ref('puja-videos/' + Date.now() + '-' + file.name);
        ...
        function renderVideos() {
            ...
            videoContainer.innerHTML = videos.map(video => `
                <div class="video-card">
                    ${isAdminLoggedIn ? 
                        `<button class="delete-btn" onclick="deleteVideo('${video.id}', '${video.url}')">
        ...
        function formatDate(dateString) {
            ...
            return isNaN(date.getTime()) ? dateString : date.toLocaleDateString('en-IN', options);
        }
